...
export const UnifiedTransactionHistory = () => {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      // Use React-PDF for professional statement generation
      const React = (await import('react')).default;
      const { pdf } = await import('@react-pdf/renderer');
      const { StatementDocument } = await import('./PDFStatement');
      
      // Calculate summary data
      const summaryData = {
        totalCount: freshData.transactions.length,
        totalCredit: freshData.transactions
          .filter(t => t.type === 'credit')
          .reduce((sum, t) => sum + t.amount, 0),
        totalDebit: freshData.transactions
      ...
      const pdfDocument = React.createElement(StatementDocument, {
        userInfo: freshData.userInfo,
        currentBalance: freshData.balance,
      ...
      link.href = url;
      link.download = `QuickPe_Statement_${freshData.userInfo.firstName}_${new Date().toISOString().split('T')[0]}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      URL.revokeObjectURL(url);
      
      console.log('✅ Professional PDF statement downloaded successfully');
